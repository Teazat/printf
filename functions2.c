...
int custom_index = CUSTOM_BUFF_SIZE - 2, custom_length = 2, custom_padd_start = 1; /* custom_length=2, for '0x' */
...
custom_width, custom_flags, custom_padd, custom_extra_c, custom_padd_start));
}
/************************* CUSTOM PRINT NON PRINTABLE *************************/
/**
* custom_print_non_printable - Custom print ASCII codes in hexadecimal of non-printable characters
* @custom_args: List of arguments
* @custom_buffer: Buffer array to handle print
* @custom_flags: Calculate active flags
* @custom_width: Get width
...
custom_offset += custom_append_hexa_code(custom_str[custom_i], custom_buffer, custom_i + custom_offset);
custom_i++;
}
